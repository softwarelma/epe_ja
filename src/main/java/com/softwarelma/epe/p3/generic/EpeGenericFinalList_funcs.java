...
public final class EpeGenericFinalList_funcs extends EpeGenericAbstract {
    ...
    public static List<String> retriveListFuncAll() throws EpeAppException {
        List<String> listPacks = EpeGenericFinalList_packs.retriveListPack();
        List<String> listFunc = new ArrayList<>();

        for (String pack : listPacks) {
            listFunc.addAll(retriveListFunc(pack));
        }
    ...
    public static List<String> retriveListFunc(String pack) throws EpeAppException {
        EpeAppUtils.checkEmpty("pack", pack);
        List<String> listFunc = new ArrayList<>();
        String packP3 = "com.softwarelma.epe.p3.";
        List<Class<?>> listClass = null;
        String classStr;
        String classPrefixLowerCase = packP3 + pack + ".epe" + pack + "final";

        try {
            listClass = getClassesForPackage(packP3 + pack);
        } catch (ClassNotFoundException e) {
            throw new EpeAppException("retriveListFunc ClassNotFoundException for pack: " + pack, e);
        } catch (Exception e) {
            throw new EpeAppException("retriveListFunc " + e.getClass().getName() + " for pack: " + pack, e);
        }

        for (Class<?> clazz : listClass) {
            classStr = clazz.getName();

            if (!classStr.contains("$") && classStr.toLowerCase().startsWith(classPrefixLowerCase)) {
                listFunc.add(classStr.substring(classPrefixLowerCase.length()).toLowerCase());
            }
        }
        ...
        try {
            ...
            for (URL url = null; resources.hasMoreElements() && ((url = resources.nextElement()) != null);) {
                try {
                    ...
                    } else if (connection instanceof sun.net.www.protocol.file.FileURLConnection) {// Object){//
                        try {
